            ...
            <div class="modal-body">
                <form>
                    <div class="form-group">
                        ...
                        <input type="email" class="form-control " required="required" id="rEmail"
                               aria-describedby="emailHelp" onblur="checkEmail()">
                        <small id="rEmailHelp" class="form-text text-muted">We'll never share your email with anyone
                            else.</small>
                    </div>
                    <div class="form-group">
                        <label for="rPassword">密码</label>
                        <input type="password" minlength="6" maxlength="16"
                               class="form-control" required="required" id="rPassword">
                    </div>
                    <div class="form-group">
                        <label for="confirmPassword">确认密码</label>
                        <input type="password" minlength="6" maxlength="16" required="required"
                               class="form-control" id="confirmPassword" onblur="checkPwd()">
                        <small id="rPwdConfirmHelp" class="form-text text-muted">123123</small>
                    </div>
                    <div class="form-group">
                        <label for="checkCode">验证码：</label>
                        <div class="row">
                            <input type="text" class="form-control col-6 ml-3" id="checkCode">
                            <img class="col-4" style="cursor: pointer" src="http://localhost:8080/vCode" onclick="changeVCode(this)"/>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">取消</button>
                        <input type="submit" class="btn btn-primary" id="registButton" value="注册" disabled="disabled"/>
                    </div>
                </form>
...
<script type="application/javascript">
    ...
    function checkPwd() {
        let pwdEle = document.getElementById("rPassword");
        let pwdConfirmEle = document.getElementById("confirmPassword");
        let help = document.getElementById("rPwdConfirmHelp");
        if(pwdConfirmEle.value != pwdEle.value ){
            help.innerText = "两次密码不一致，请检查或重新输入"
